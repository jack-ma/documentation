...
After completing the bsp environment configuration, you need to compile the kernel once before pulling the kernel code from the repository, you can execute `. /bsp linux rk356x` to build the kernel, the path to the kernel source code is located in `.src/linux` in the bsp directory.

:::info
The second argument to the bsp command is the kernel repository or branch to use, and the corresponding arguments for each SBC can be found by running `uname -r` to see the kernel suffix.
:::

After modifying the kernel source code, the kernel can be built again:

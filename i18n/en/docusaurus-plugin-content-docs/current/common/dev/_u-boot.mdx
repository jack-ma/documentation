...
:::info
`bsp` The recommended environment is Debian or a Debian-based Linux distribution running on a 64-bit X86 architecture computer.
:::.
...
Take Radxa ZERO 3 as an example, you can run `sudo apt list --installed | grep u-boot` on the development board to get the U-Boot package name `u-boot-latest`.  

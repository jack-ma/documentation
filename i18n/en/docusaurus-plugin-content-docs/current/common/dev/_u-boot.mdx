...
[bsp](https://github.com/radxa-repo/bsp) is a set of tools provided by Radxa to quickly build a U-Boot Kernel. It is very easy to build your own U-Boot and Kernel using bsp.
...
Run `. /bsp` will show all the parameters and their explanations.
After completing the bsp environment configuration, you need to compile the kernel once before pulling the kernel code from the repository, you can execute `. /bsp u-boot latest radxa-zero3` to build the kernel, the path to the U-Boot source code is located in `.src/u-boot/` in the bsp directory.

:::info
The second parameter of the bsp command is the U-Boot repository or branch to use, and the corresponding parameter for each SBC can be found by running `sudo apt list --installed |grep u-boot` to see the u-boot suffix.
The third parameter is the SBC model
:::.

U-Boot can be built again after modifying the U-Boot source code:

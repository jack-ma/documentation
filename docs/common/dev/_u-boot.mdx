...
:::info
`bsp` 推荐使用环境为运行在 64 位 X86 架构计算机上的 Debian 或者基于 Debian 的 Linux 发行版。
:::
...
以 Radxa ZERO 3 为例，可以在 SBC 上执行 `sudo apt list --installed | grep u-boot` 获取 U-Boot 包名为 `u-boot-latest`。  

...
请参考[bsp 环境配置](https://radxa-repo.github.io/bsp)，进行环境配置。
直接运行 `./bsp` 会显示所有参数及其解释。
完成 bsp 环境配置后，需要先编译一次内核才会从仓库拉取内核代码，可以执行 `./bsp u-boot latest radxa-zero3` 构建内核, U-Boot 源码的路径位于 bsp 目录下的 `.src/u-boot/`。

:::info
bsp 命令的第二个参数为使用的 U-Boot 仓库或分支，各个 SBC 具体对应的参数可以运行 `sudo apt list --installed |grep u-boot` 查看 u-boot 后缀。
第三个参数为 SBC型号
:::

在修改 U-Boot 源码后，可再次构建 U-Boot ：
